...
export async function cacheGet(key: string): Promise<any | null> {
  ...
  try {
    ...
    // Silently fail on network errors - cache is optional
    console.warn('[cache] Failed to get from cache:', error instanceof Error ? error.message : 'Unknown error');
    return null;
  }
}
...
export async function cacheSet(key: string, value: any, ttlSeconds = 60): Promise<boolean> {
  ...
  try {
    ...
    // Silently fail on network errors - cache is optional
    console.warn('[cache] Failed to set cache:', error instanceof Error ? error.message : 'Unknown error');
    return false;
  }
}
